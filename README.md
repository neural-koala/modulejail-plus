...
I can't find a list of obsolete built-in modules from the Internet, so I've manually compiled a list of built-in modules that you can safely disable based on the `modules.builtin` file of my Ubuntu 26.04 machine, if you are running a modern desktop/laptop/server, and do not need ancient things like CDROM (this list is not generated by AI).
```
